...
class NexusPhpSiteUserInfo(_ISiteUserInfo):
    ...
    def __parse_user_traffic_info(self, html_text):
        ...
        if self.site_url.find('hhanclub') != -1:
            ratio_match = re.search("分享率][:：_<>/a-zA-Z-=\"'\s#;]+([\d,.\s]+)",html.xpath('//*[@id="user-info-panel"]/div[2]/div[1]/div[1]/div/text()')[0])
        # 计算分享率
